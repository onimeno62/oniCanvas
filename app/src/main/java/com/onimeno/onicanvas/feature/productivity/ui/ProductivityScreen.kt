                ...
                Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(6.dp)) {
                    ProductivityTool.values().forEach { tool ->
                        FilterChip(
                            ...
                            modifier = Modifier.weight(1f).testTag("productivity_mode_${tool.name.lowercase()}")
                        )
                    }
                }
            }
...
@Composable private fun StatusCard(connected: Boolean) {
    Card(Modifier.fillMaxWidth(), colors = CardDefaults.cardColors(
        ...
        Row(Modifier.padding(16.dp), verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(12.dp)) {
            ...
            Column {
                Text(if (connected) "Companion connected" else "Companion disconnected", fontWeight = FontWeight.Bold)
                Text(if (connected) "Productivity commands are live" else "Connect before using controls", style = MaterialTheme.typography.bodySmall)
            }
        }
    }
}
...
@Composable private fun RadialPanel(connected: Boolean, vm: ProductivityViewModel) {
    val actions = listOf(
        ProductivityAction.UNDO to Icons.Rounded.Undo, ProductivityAction.REDO to Icons.Rounded.Redo,
        ProductivityAction.BRUSH to Icons.Rounded.Brush, ProductivityAction.ERASER to Icons.Rounded.Delete,
        ProductivityAction.SAVE to Icons.Rounded.Save, ProductivityAction.TRANSFORM to Icons.Rounded.Transform,
        ProductivityAction.SELECT to Icons.Rounded.SelectAll, ProductivityAction.COPY to Icons.Rounded.ContentCopy
    )
    Column(horizontalAlignment = Alignment.CenterHorizontally) {
        ...
        Box(Modifier.fillMaxWidth().height(320.dp).testTag("radial_menu_surface"), contentAlignment = Alignment.Center) {
            Card(Modifier.size(92.dp), shape = CircleShape, colors = CardDefaults.cardColors(containerColor = MaterialTheme.colorScheme.primaryContainer)) {
                Box(Modifier.fillMaxSize(), contentAlignment = Alignment.Center) { Icon(Icons.Rounded.Menu, contentDescription = "Radial menu") }
            }
            actions.forEachIndexed { index, pair ->
                val angle = Math.toRadians(index * 45.0 - 90.0)
                val x = (kotlin.math.cos(angle) * 105.0).toFloat()
                val y = (kotlin.math.sin(angle) * 105.0).toFloat()
                androidx.compose.material3.IconButton(
                    onClick = { vm.perform(pair.first) }, enabled = connected,
                    modifier = Modifier.align(Alignment.Center).offset(x.dp, y.dp).size(54.dp)
                        .background(MaterialTheme.colorScheme.surfaceVariant, CircleShape)
                        .testTag("radial_${pair.first.name.lowercase()}")
                ) { Icon(pair.second, contentDescription = pair.first.name) }
            }
        }
    }
}
...
@Composable private fun TouchpadPanel(connected: Boolean, vm: ProductivityViewModel) {
    ...
    Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
        ...
        Box(
            Modifier.fillMaxWidth().height(300.dp).background(MaterialTheme.colorScheme.surfaceVariant, RoundedCornerShape(20.dp))
                .pointerInput(connected, sensitivity) { detectDragGestures { _, amount -> vm.mouseMove((amount.x * sensitivity).toDouble(), (amount.y * sensitivity).toDouble()) } }
                .pointerInput(connected) { detectTapGestures(onTap = { vm.mouseButton("left", true); vm.mouseButton("left", false) }, onLongPress = { vm.mouseButton("right", true); vm.mouseButton("right", false) }) }
                .testTag("touchpad_surface"),
            contentAlignment = Alignment.Center
        ) {
            Column(horizontalAlignment = Alignment.CenterHorizontally) {
                Icon(Icons.Rounded.Mouse, contentDescription = null, modifier = Modifier.size(48.dp))
                Text(if (connected) "Touchpad active" else "Touchpad disabled", fontWeight = FontWeight.Bold)
                Text("Drag • tap • long press", textAlign = TextAlign.Center, style = MaterialTheme.typography.bodySmall)
            }
        }
        Row(verticalAlignment = Alignment.CenterVertically) {
            Text("Sensitivity")
            Slider(sensitivity, { sensitivity = it }, valueRange = 0.5f..3f, Modifier.weight(1f).testTag("touchpad_sensitivity"))
        }
        Button(onClick = { vm.scroll(0.0, -5.0) }, enabled = connected, Modifier.fillMaxWidth().testTag("touchpad_scroll_up")) { Text("Scroll Up") }
        Button(onClick = { vm.scroll(0.0, 5.0) }, enabled = connected, Modifier.fillMaxWidth().testTag("touchpad_scroll_down")) { Text("Scroll Down") }
    }
}

@Composable private fun LayerPanel(connected: Boolean, vm: ProductivityViewModel) {
    val actions = listOf(
        ProductivityAction.NEW_LAYER to "New Layer", ProductivityAction.DUPLICATE_LAYER to "Duplicate Layer",
        ProductivityAction.MERGE_LAYER to "Merge Layer", ProductivityAction.LOCK_LAYER to "Lock Layer",
        ProductivityAction.TOGGLE_MASK to "Toggle Mask", ProductivityAction.OPACITY_DOWN to "Opacity −",
    ...
    Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        actions.forEach { (action, label) ->
            Button(onClick = { vm.perform(action) }, enabled = connected, Modifier.fillMaxWidth().testTag("layer_${action.name.lowercase()}")) {
                Icon(Icons.Rounded.Layers, contentDescription = null); Spacer(Modifier.size(8.dp)); Text(label)
            }
        }
    }
}
        ...
        favorites.chunked(2).forEachIndexed { row, pair ->
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                pair.forEachIndexed { column, name ->
                    OutlinedCard(onClick = { vm.brushPreset(row * 2 + column) }, enabled = connected, Modifier.weight(1f).testTag("brush_favorite_${row * 2 + column}")) {
                        Column(Modifier.fillMaxWidth().padding(14.dp), horizontalAlignment = Alignment.CenterHorizontally) {
                            Icon(Icons.Rounded.Brush, contentDescription = null); Spacer(Modifier.height(4.dp)); Text(name, fontWeight = FontWeight.Bold)
                        }
                    }
                }
            }
...
private fun ProductivityTool.label() = when (this) {
    ProductivityTool.RADIAL_MENU -> "Radial"
    ProductivityTool.TOUCHPAD -> "Touchpad"
    ProductivityTool.LAYERS -> "Layers"
    ProductivityTool.BRUSHES -> "Brushes"
}
